...
function mapDbRow(r: DbSkeletonRow): SkeletonRow {
  return {
    ...
    relatedView: r.related_view === 'inline' || r.related_view === 'popover' ? r.related_view : 'none',
  };
}
...
export async function loadAppendixSkeleton(): Promise<SkeletonRow[]> {
  const { data, error } = await supabase
    ...
    .select('row_id, section_id, section_title, legal_basis, condition_tested, effect, kind, related_view, allowed_states, driven_by_question_ids, render_if, sort_order')
    .eq('is_active', true)
