...
function mapDbRow(r: DbSkeletonRow): SkeletonRow {
  return {
    ...
    allowedStates: (Array.isArray(r.allowed_states) ? r.allowed_states : []) as Decision[],
    ...
    flags: (Array.isArray(r.flags) ? r.flags : undefined) as SkeletonRow['flags'],
  };
}
...
export async function loadAppendixSkeleton(): Promise<SkeletonRow[]> {
  const { data, error } = await supabase
    ...
    .select('row_id, section_id, section_title, legal_framework, effect, allowed_states, driven_by_question_ids, render_if, flags, sort_order')
    .eq('is_active', true)
