// Deterministic factsheet combine (canonical / frontend copy).
//
// DUAL MAINTENANCE — keep IN SYNC with the Deno copy at
// supabase/functions/_shared/factsheetMerge.ts (bodies identical; only the
// Factsheet import path differs). Same rule as factsheetSchema.ts.
//
// build-factsheet merges a big dossier by CHUNKING the documents, letting the
// model merge each chunk into a partial fact sheet (fast, fits the edge
// wall-clock), then combining the partials HERE, in code, with no model call —
// so the fact sheet always completes even when a single 19-document merge would
// time out. Cross-document joins WITHIN a chunk are the model's; cross-chunk
// joins are the entity dedup below (TIN + name/alias).
