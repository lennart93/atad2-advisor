-- Swarm prompt v16: GROUP OF SEPARATE TAXPAYERS (terminology + concept fix on
-- top of v15).
--
-- v15 fixed the single-entity COLLAPSE (with several named entities the model
-- addressed only one), but it framed the group with fiscal-unity language: "ALL
-- of them together are the taxpayer under review for this single assessment".
-- That is the wrong mental model, and Lennart flagged it: a fiscal unity is a
-- group of entities treated as ONE taxpayer, which is NOT what the multi-entity
-- intake means. Here each named entity is a SEPARATE Dutch taxpayer; they are
-- batched into one dossier/memo for convenience only. Being listed together must
-- never imply a single consolidated taxpayer, and whether any of them actually
-- form a fiscal unity is a separate question answered from the documents, never
-- assumed from the batching. (The UI note was corrected in the same change:
-- "Assessed together as one taxpayer" -> "... as one group of taxpayers" in
-- src/pages/Assessment.tsx.)
--
-- v16 rewrites ONLY the opening clause of Rule 2 to say "several SEPARATE
-- taxpayers, assessed together as a group, not a fiscal unity by construction".
-- The collective-presentation guidance from v15 (state a common fact once for
-- the group, e.g. "each qualifies as a Dutch domestic corporate income taxpayer
-- under Article 2") is unchanged and is already correct (it says EACH qualifies,
-- i.e. separate taxpayers). Everything else from v15/v14 (Route A/B routing,
-- Route B trio invariant, jurisdictional sanity Rule 12, <=450-char
-- client_question, strict grounding, no statute references, banned dashes, the
-- labelled taxpayer_name template, JSON shape) is byte-for-byte unchanged. No
-- shape change, so the prefill-documents EDGE FUNCTION DOES NOT NEED A REDEPLOY
-- and compose_client_letter needs no change.
--
-- Model: claude-opus-4-8, matching the live active row (do NOT regress to 4-7).
--
-- Re-runnable: the UPDATE only demotes versions below 16, the INSERT is WHERE
-- NOT EXISTS guarded (UNIQUE (key, version)), and a trailing UPDATE re-asserts
-- v16 active on rerun.

UPDATE atad2_prompts
SET is_active = false
WHERE key = 'prefill_swarm_system' AND is_active = true AND version < 16;
...
2. TAXPAYER IS GIVEN, NOT GUESSED, AND MAY BE SEVERAL SEPARATE TAXPAYERS. The Assessment context block at the top of the user message states the taxpayer_name and fiscal_year that the user already entered on the Assessment page. That taxpayer_name is AUTHORITATIVE for this assessment. It may name ONE entity or SEVERAL, listed one per line. When several are listed, EACH named entity is a SEPARATE Dutch taxpayer in its own right; they are assessed together as one group in this single assessment for convenience (one dossier, one memo), NOT because they are a single consolidated taxpayer. Being listed together is NOT, by itself, a fiscal unity: do not treat the group as one taxpayer, do not assume intra-group transactions net out or disappear, and assess each entity's position on its own. Whether any of the named entities actually form a fiscal unity (or are otherwise consolidated) is a separate question to be answered from the documents, never assumed from the fact that they are assessed together. You MUST:
   - Treat every named entity (all of them, not only the first) as a Dutch taxpayer under review, even when the documents (including image-only structure charts) show additional Dutch entities or do not literally mention a name. Do NOT collapse a multi-entity list to a single entity, do NOT silently address only the first, do NOT drop the others. Do NOT pick a different entity than those named. Do NOT hedge about which entities are the taxpayer. Do NOT ask the user to confirm which entity is the Dutch taxpayer.
   - COVER THE WHOLE SET, COLLECTIVELY. When a fact or conclusion holds for every named entity, state it ONCE for the group and make clear it applies to all of them, rather than repeating it entity by entity. Example: "The named entities are all Dutch private limited companies (B.V.s) wholly held by WMC Energy B.V., and each qualifies as a Dutch domestic corporate income taxpayer under Article 2 of the Dutch Corporate Income Tax Act." Only when the documents genuinely differentiate a specific entity (a transaction, counterparty, jurisdiction, instrument or classification that applies to some but not all) do you name that entity explicitly and state the distinction. Never write an output that speaks about only one named entity when the question applies to several, and never invent per-entity differences the documents do not support.
   - Frame all outputs (suggested_toelichting, suggested_toelichting_unknown, answer_rationale, contextual_hint, client_question) from the perspective of the taxpayer set, opening with a natural reference to the group (e.g. "The named entities...", "The taxpayer group...") or to the specific entities the point concerns (client_question instead follows the We-understand pattern from Rule 0). With a single named entity this reduces to that one entity, exactly as before: begin every output with that entity's name where natural.
...
13. FINAL CHECK before emitting JSON. Verify:
    ...
    - When the Assessment context names several taxpayer entities, the output covers all of them (a common fact stated once for the group), not just one; an output that discusses only the first named entity while the question applies to the whole set is wrong.
...
taxpayer_name (one entity per line; when several are listed, all named entities together are the taxpayer under review):
{{taxpayer_name}}
...
Output the JSON suggestion now.$template$,
  ...
  4000,
  true,
  'v16: v15 plus GROUP-OF-SEPARATE-TAXPAYERS terminology/concept fix. v15 solved the single-entity collapse but framed the group as one taxpayer ("all of them together are the taxpayer under review"), which is fiscal-unity language. Lennart flagged it: a fiscal unity is a group treated as ONE taxpayer, which is NOT the multi-entity intake''s meaning. v16 rewrites only the opening clause of Rule 2: each named entity is a SEPARATE Dutch taxpayer, assessed together as one group for convenience (one dossier, one memo), NOT a single consolidated taxpayer; being listed together is not by itself a fiscal unity, intra-group items are not assumed to net out, each entity is assessed on its own, and whether they actually form a fiscal unity is answered from the documents not assumed from the batching. The v15 collective-presentation guidance (state a common fact once for the group, "each qualifies under Article 2") is unchanged and already correct (EACH qualifies = separate taxpayers). UI note corrected in the same change ("as one taxpayer" -> "as one group of taxpayers", src/pages/Assessment.tsx). Everything else from v15/v14 byte-for-byte unchanged (Route A/B routing, Route B trio invariant, jurisdictional sanity Rule 12, 450-char client_question cap, strict grounding, no statute references, banned dashes, labelled taxpayer_name template, JSON shape). No shape change, so no edge-function redeploy and no compose_client_letter change. Model claude-opus-4-8 to match the live active row. Re-runnable: demote versions below 16, INSERT guarded by WHERE NOT EXISTS, trailing UPDATE re-asserts v16 active.'
WHERE NOT EXISTS (
  SELECT 1 FROM atad2_prompts
  WHERE key = 'prefill_swarm_system' AND version = 16
);

-- Re-assert v16 active on rerun (the INSERT only sets is_active on first insert).
