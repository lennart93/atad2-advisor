-- appendix_facts_system v16. Apply on the VM as supabase_admin.
-- Builds on v15. The LOCAL (home-state) classification becomes strictly
-- evidence-driven: only stated when the documents or the advisor's answer
-- explanations (new EVIDENCE_NOTES input) show the actual local treatment.
-- The v11 "always fill the local view, US LLC transparent by default" rule is
-- gone; a guessed default is worse than an open cell the advisor fills.
...
{{STRUCTURE_BLOCK}}$prompt$,
  notes = 'v16: local classification strictly evidence-driven (documents or EVIDENCE_NOTES; no guessed defaults); EVIDENCE_NOTES input added. Otherwise identical to v15.'
