-- Appendix prompt v4. Apply on the VM as supabase_admin (in-place UPDATE of the
-- single appendix_system row, like v3). Idempotent.
--
-- Changes vs v3:
--   1. Four-value status vocabulary (adds "N/A") with an explicit assignment rule:
--      scope-gate-satisfied and moot-downstream rows are "N/A", never "Triggered"
--      or "Insufficient information". (The generate-appendix edge function also
--      enforces this deterministically via mootness.ts; the prompt makes the
--      model emit it directly and the code is the backstop.)
--   2. Copy length and tone: tight cells, one sentence for N/A rows, no
--      "if X then Y" hypotheticals on N/A rows, no filler, no restating a fact
--      twice, direct phrasing instead of blanket "we understand that" hedging.
--   3. Related-party derivation: use ONLY the >25% (50% hybrid) set from
--      ESTABLISHED FACTS; never call a sub-25% lender/co-investor/service provider
--      related; name the qualifying related party on the relatedness rows.
--   4. Banned phrasing: "heads a fiscal unity" / "is the head of a fiscal unity"
--      -> "is the parent company of a fiscal unity".
--   5. Adds the ESTABLISHED FACTS ({{FACTS_BLOCK}}) and EVIDENCE_NOTES inputs the
--      edge function already fills, so relatedness and mismatch reasoning is
--      grounded on the deterministic entity register.
--
-- Requires the skeleton allowed_states to include "N/A" first
-- (20260617140000_appendix_skeleton_v4_na_state.sql), otherwise the edge function
-- coerces an "N/A" back to "Insufficient information".
...
{"rows":[{"rowId":"<id>","status":"<one allowed state>","reasoning":"<tight narrative>","provenance":"<evidence or empty string>"}]}
...
REMINDER: output ONLY the JSON object with one entry per skeleton row. status must be one of that row's allowedStates and must follow STATUS ASSIGNMENT: a satisfied scope gate is "N/A", a moot downstream row is "N/A", "Triggered" only when a mismatch fires, "Insufficient information" only for a reachable row with a missing fact. Keep internal codes out of reasoning and put them in provenance. Keep cells tight.$prompt$,
  notes = 'v4: four-value status with N/A (scope-gate-satisfied + moot-downstream), tight copy/tone (one sentence for N/A, no hypotheticals, no filler, direct phrasing, banned "head of a fiscal unity"), related-party set = >25% only from ESTABLISHED FACTS, added FACTS_BLOCK + EVIDENCE_NOTES inputs. Pairs with skeleton allowed_states v4 and the mootness.ts safety net in the edge function.'
