-- appendix_facts_system v7. Apply on the VM as supabase_admin.
-- Makes Part A (the facts) answer-independent: the ANSWERS_BLOCK is dropped from
-- the inputs and the intro no longer mentions the assessment answers. Part A is
-- derived from the documents + structure chart + grounded knowledge base, not the
-- questionnaire, so the refine pass can reuse it whenever the structure and
-- documents are unchanged (only Part B re-runs against the answers). Everything
-- else is identical to v6 (fiscalUnityMemberEntityIds, KB grounding, hard rules,
-- output format, all other placeholders). Overwrites the active row in place.
...
{{STRUCTURE_BLOCK}}$prompt$,
  notes = 'v7: drops the answers block; Part A is answer-independent (facts from documents + structure + KB). Otherwise identical to v6.'
