-- Swarm prompt v15: MULTI-ENTITY TAXPAYER coverage.
-- Observed failure (Helios dossier, 9 named taxpayer entities in one
-- assessment): every suggested_toelichting and client_question spoke about a
-- single entity (the first one, Helios I B.V.) as if it were the only taxpayer,
-- e.g. "Helios I B.V. is a private limited company ... it qualifies as a Dutch
-- domestic corporate income taxpayer under Article 2", ignoring the other eight.
-- The multi-taxpayer intake stores every named entity in atad2_sessions
-- .taxpayer_name newline-joined (src/lib/taxpayer.ts formatTaxpayerNames), and
-- buildDocumentsBlock passes that raw value into {{taxpayer_name}}. But v14
-- Rule 2 ("TAXPAYER IS GIVEN, NOT GUESSED") is written entirely in the SINGULAR
-- ("the named entity", "that entity's perspective", "begin every output with
-- that entity's name", "Do NOT pick a different entity"), so the model collapses
-- a multi-line list to one entity and drops the rest.
--
-- v15 rewrites ONLY Rule 2 to be multi-entity aware, and labels the taxpayer_name
-- block in the user template so a multi-line value reads as a set rather than as
-- one messy value bleeding into the next line. Behaviour for a SINGLE named
-- entity is unchanged (the new rule explicitly reduces to the old one). Answer
-- style per Lennart: COLLECTIVE, split when facts differ, so a fact common to all
-- entities is stated once for the group (which also keeps outputs inside the
-- 1000/450-char caps instead of enumerating nine entities). Everything else from
-- v14 (Route A/B routing, the Route B trio invariant, jurisdictional sanity,
-- <=450 char client_question, strict grounding, no statute references, banned
-- dashes, JSON shape) is byte-for-byte unchanged. No new field and no shape
-- change, so the prefill-documents EDGE FUNCTION DOES NOT NEED A REDEPLOY, and
-- the compose_client_letter step needs no change (it only merges the per-question
-- drafts, which now already carry every entity).
--
-- Model: claude-opus-4-8, matching the live active row after
-- 20260704130000_bump_models_opus48_sonnet5.sql (do NOT regress to 4-7).
--
-- Re-runnable: the UPDATE only demotes versions below 15, the INSERT is WHERE
-- NOT EXISTS guarded (UNIQUE (key, version)), and a trailing UPDATE re-asserts
-- v15 active on rerun.

UPDATE atad2_prompts
SET is_active = false
WHERE key = 'prefill_swarm_system' AND is_active = true AND version < 15;
...
2. TAXPAYER IS GIVEN, NOT GUESSED, AND MAY BE SEVERAL ENTITIES. The Assessment context block at the top of the user message states the taxpayer_name and fiscal_year that the user already entered on the Assessment page. That taxpayer_name is AUTHORITATIVE for this assessment. It may name ONE entity or SEVERAL, listed one per line; when several are listed, ALL of them together are the taxpayer under review for this single assessment. You MUST:
...
Output the JSON suggestion now.$template$,
  ...
  4000,
  true,
  'v15: v14 plus MULTI-ENTITY TAXPAYER coverage. Rule 2 rewritten from singular to set-aware: taxpayer_name may name one entity or several (one per line, from the multi-taxpayer intake stored newline-joined in atad2_sessions.taxpayer_name). All named entities together are the taxpayer under review; the model must cover the whole set collectively (a fact common to all stated once for the group, e.g. "The named entities are all Dutch B.V.s wholly held by WMC Energy B.V., and each qualifies under Article 2 of the Dutch Corporate Income Tax Act"), naming a specific entity only where the documents genuinely differentiate it, and must never speak about only the first entity when the question applies to several. Single-entity behaviour is unchanged (rule explicitly reduces to v14). The user template now labels the taxpayer_name block so a multi-line value reads as a set. Observed failure on the Helios dossier (9 taxpayer entities): every suggested_toelichting and client_question addressed only Helios I B.V. Everything else from v14 is byte-for-byte unchanged (Route A/B routing, Route B trio invariant, jurisdictional sanity Rule 12, 450-char client_question cap, strict grounding, no statute references, banned dashes, JSON shape). No new field and no shape change, so the prefill-documents edge function does NOT need a redeploy, and compose_client_letter needs no change (it only merges the per-question drafts). Model claude-opus-4-8 to match the live active row after the 2026-07-04 model bump. Re-runnable: demote versions below 15, INSERT guarded by WHERE NOT EXISTS, trailing UPDATE re-asserts v15 active.'
WHERE NOT EXISTS (
  SELECT 1 FROM atad2_prompts
  WHERE key = 'prefill_swarm_system' AND version = 15
);

-- Re-assert v15 active on rerun (the INSERT only sets is_active on first insert).
