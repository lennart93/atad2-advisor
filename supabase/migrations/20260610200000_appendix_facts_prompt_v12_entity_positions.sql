-- appendix_facts_system v12. Apply on the VM as supabase_admin.
-- Builds on v11. Adds positionByEntityId: for every Group entity (no parent or
-- subsidiary role), one short grounded clause on how it relates to the taxpayer
-- (e.g. "co-investor in a named fund alongside E9"), shown in the register's Role
-- column. Strictly from the documents; omitted when the documents give nothing.
-- Note: natural persons are filtered out of the register server-side unless they
-- are genuinely associated, so the model no longer sees them.
...
2. positionByEntityId: for EVERY entity whose role in the ENTITY_REGISTER is "Group entity" (not Taxpayer, Parent or Subsidiary), one SHORT factual clause (max ~12 words) describing how that entity relates to the taxpayer, grounded STRICTLY on the SOURCE_DOCUMENTS and the STRUCTURE_BLOCK. Examples: "co-investor in Energiefonds Overijssel I alongside E9", "pension foundation connected to the group", "external fund manager of E12". Omit an entity entirely when the documents do not show the relationship; never guess.
3. fiscalUnityMemberEntityIds: the entity ids (E2, E3 ...) that, on the evidence in the SOURCE_DOCUMENTS, form a Dutch fiscal unity (fiscale eenheid voor de vennootschapsbelasting) together with the taxpayer E1, and are therefore part of the same Dutch taxpayer. A fiscal unity requires Dutch resident corporate taxpayers in which (directly or indirectly) at least 95% of the shares are held within the group. Include an entity ONLY when the documents indicate it sits in the fiscal unity with E1 (for example it is named in a fiscale-eenheid beschikking/besluit, the financial statements state a CIT fiscal unity, or the documents otherwise establish it). Never include a foreign entity, a transparent entity, or an entity for which the documents give no fiscal-unity indication. Return an empty array when the documents do not establish a fiscal unity. E1 is implied and need not be listed.
4. actingTogether: ONE assessment of whether the parents act together as a coordinated group (samenwerkende groep), covering ALL the parents together as one group, never zooming in on a single sub-pair. Return AT MOST ONE entry, or an empty array when there are no parents to assess. For that one entry return:
...
5. transactions: the intra-group flows between related entities that the ATAD2 articles test, each with from/to entity id, kind, instrument, a short note, and which article(s) it triggers (e.g. "12aa(1)(a)"). For EACH transaction also return:
...
Return ONLY a JSON object: {"nlTaxStatusByEntityId":{...},"positionByEntityId":{...},"fiscalUnityMemberEntityIds":[...],"actingTogether":[{"memberEntityIds":[...],"combinedPct":..,"likelihood":"..","reasoning":".."}],"transactions":[{...,"relevant":true,"relevanceReason":".."}],"classifications":[...],"narratives":{"register":"..","related":"..","flows":"..","classification":".."}}
...
{{STRUCTURE_BLOCK}}$prompt$,
  notes = 'v12: positionByEntityId, one grounded relationship clause per group entity for the register. Otherwise identical to v11.'
