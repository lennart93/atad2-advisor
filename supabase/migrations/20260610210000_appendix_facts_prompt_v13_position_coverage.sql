-- appendix_facts_system v13. Apply on the VM as supabase_admin.
-- Builds on v12. The position instruction is now a per-entity obligation: work
-- through EVERY Group entity in the register and state its relationship to the
-- taxpayer whenever the documents or the structure show one (co-investor, fund
-- vehicle, lender, foundation, manager, JV partner). v12 phrased it permissively
-- and the model skipped all of them. Grounding rule unchanged: omit only when
-- there is genuinely nothing in the inputs; never guess.
...
{{STRUCTURE_BLOCK}}$prompt$,
  notes = 'v13: positionByEntityId is a per-entity obligation (v12 phrasing let the model skip them all). Otherwise identical to v12.'
