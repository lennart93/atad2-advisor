-- appendix_facts_system v18. Apply on the VM as supabase_admin.
-- Builds on v17. Reworks item 6 (actingTogether): instead of ONE assessment of
-- all parents lumped together (at most one entry), the model now enumerates the
-- plausible acting-together groupings among the taxpayer's parents and direct
-- shareholders, returning ONE entry per grouping. A shareholder that already
-- holds >25% on its own does NOT crowd the section out: other shareholders can
-- still form a group on their own, or together with that large shareholder. The
-- model must always return at least the most plausible grouping it considered
-- (even when assessed unlikely), so the appendix shows what was weighed; it
-- returns an empty array only when there are fewer than two parents/direct
-- shareholders to assess at all. Pairs with index.ts dropping the .slice(0,1)
-- cap so every returned grouping is kept and renumbered A1, A2, ...
...
{{STRUCTURE_BLOCK}}$prompt$,
  notes = 'v18: actingTogether enumerates plausible groupings (one entry per grouping), a >25% parent no longer crowds out other shareholder groupings, always returns at least the most plausible grouping considered. Pairs with index.ts dropping the single-cluster cap. Otherwise identical to v17.'
