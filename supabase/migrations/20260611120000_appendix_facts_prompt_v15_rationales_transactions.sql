-- appendix_facts_system v15. Apply on the VM as supabase_admin.
-- Builds on v14:
-- (1) Acting-together returns a self-contained assessment text for EVERY
--     likelihood level (rationales), so the advisor can switch levels in the
--     app and the text follows without a new AI call.
-- (2) nlTaxStatusReasonByEntityId is its own numbered obligation (it was buried
--     inside item 1 and the model regularly skipped it).
-- (3) Wording: "flow(s)" reads as "transaction(s)" throughout.
...
8. classifications: how an entity is treated in its home state vs the relevant counterparty/source state (transparent, opaque or disregarded), and whether that is a mismatch (hybrid=true when home and source differ). Return a classification row for EVERY entity that is (a) a party to a transaction you marked relevant, (b) a related party established outside the Netherlands, or (c) of a legal form that is prone to classification differences regardless of jurisdiction (LLC, LP, CV-like or other partnership, trust, Stiftung, foundation). Always fill homeState and homeClass (use the GROUNDED_LITERATURE; "transparent" or "opaque"); never leave the local view open for these entities. A US LLC, for example, must always get a row stating its US treatment (transparent by default unless documents show a corporate election) against the Dutch view.
...
{{STRUCTURE_BLOCK}}$prompt$,
  notes = 'v15: acting-together rationales per level (level switch swaps text in the app); nlTaxStatusReasonByEntityId promoted to its own obligation; flow wording is transaction. Otherwise identical to v14.'
