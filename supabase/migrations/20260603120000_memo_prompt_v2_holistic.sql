-- v2: holistic analysis discipline for the memo prompt.
--
-- Problem observed under v1 (Precision Global Corporation Netherlands B.V.
-- memo, FY2024): when the model spots a high-salience hook (here: a US
-- corporate parent), it tunnels on that one angle and forgets:
--   1. The symmetric counterpart analysis. It asked only for the US tax
--      classification of the Dutch entity and its subs, never for the
--      local-jurisdiction classification of the same subs (Ireland for the
--      DAC, Germany for the GmbH, etc.). A hybrid-entity analysis is
--      structurally two-sided; one side alone is incomplete.
--   2. Known jurisdictional defaults. It hedged on a reverse-hybrid concern
--      for the Dutch BV without applying the obvious baseline that a BV is
--      non-transparent in NL by default, which already closes that branch.
--   3. The full set of cross-border relationships. It pinned everything on
--      the US parent relationship and treated the nine foreign subsidiaries
--      as a single block. Direct vs. indirect holdings were conflated.
--
-- v2 changes (insertions only, no removals from v1):
--   - New "Analytical discipline" block in the ATAD2 technical assessment
--     section, inserted between the Structure rules and the Style rules.
--     Contains two sub-blocks:
--       Block A (default classifications): forces the model to apply
--         known-default treatments (NL BV non-transparent, GmbH
--         non-transparent under DE law, DAC non-transparent under IE law,
--         Delaware LLC disregarded for US absent election) and to use
--         those defaults to close one side of a two-sided question.
--       Block B (holistic sweep): forces enumeration of every cross-border
--         relationship before narrowing, and forces a direct vs. indirect
...
'v2: adds Analytical discipline block to the ATAD2 technical assessment section. Forces application of known-default classifications (NL BV non-transparent, GmbH non-transparent under DE law, DAC non-transparent under IE law, Delaware LLC disregarded by US default) and a holistic sweep over every cross-border relationship before narrowing. Fixes tunnel-vision pattern observed on Precision Global FY2024: model fixated on US-parent classification, skipped local-jurisdiction classification of subs, mishandled reverse-hybrid direction, and conflated direct/indirect holdings.'
);
