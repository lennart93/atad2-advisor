-- appendix_facts_system v11. Apply on the VM as supabase_admin.
-- Builds on v10 with two corrections from advisor review:
-- (1) Relevance: "cross-border" strictly means the two parties are established in
--     DIFFERENT jurisdictions. A domestic NL-NL flow is never cross-border, even
--     when the recipient sits outside the scope of Dutch CIT (a Dutch foundation,
--     association or pension fund). Non-taxation alone is not an ATAD2 mismatch;
--     a hybrid mismatch needs a qualification difference between two states.
-- (2) Classifications: a row is now REQUIRED for every entity that is party to a
--     relevant flow, every related party established outside the Netherlands, and
--     every entity with a classification-sensitive legal form (LLC, LP/CV-like
--     partnership, trust, Stiftung), with homeState and homeClass always filled.
...
{{STRUCTURE_BLOCK}}$prompt$,
  notes = 'v11: cross-border strictly = different jurisdictions (domestic outside-CIT recipients are not cross-border); classification rows required for relevant parties, foreign related parties and classification-sensitive legal forms. Otherwise identical to v10.'
