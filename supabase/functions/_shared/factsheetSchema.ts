// CANONICAL factsheet schema (Deno / edge-function copy).
//
// DUAL MAINTENANCE — keep IN SYNC with the frontend canonical at
// `src/lib/factsheet/schema.ts`. Same rule as skeletonRows.ts / mootness.ts:
// any change in one file must be mirrored in the other. The bodies are
// byte-identical; only these header comments differ.
//
// Imported by extract-docfacts and build-factsheet via `../_shared/...`.
// DEPLOY NOTE: this `_shared` dir MUST be rsynced to the VM alongside those two
// function dirs (see supabase/deploy/deploy_factsheet_phase2.sh), otherwise the
// relative import fails at container start.
//
// The schema is DELIBERATELY LENIENT: LLM output varies, and a slightly
// off-shape merge must still store rather than 500 a whole factsheet.
