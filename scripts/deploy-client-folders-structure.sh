#!/bin/bash
# Apply slice-4 STRUCTURE migration (client folders). Backfill is separate and
# applied only after the owner approves the folder list.
set -e
...
md5sum /tmp/client-folders/20260610200100_client_folders.sql
echo "=== APPLYING 20260610200100_client_folders.sql (single transaction) ==="
docker exec -i "$DB" psql -U supabase_admin -d postgres -1 -v ON_ERROR_STOP=1 < /tmp/client-folders/20260610200100_client_folders.sql
